...
try:  # Neutrino exists only in the Neutrino fork
...
def test_toy_exactness(optimizer_name, bf16):
    if optimizer_name == "neutrino" and get_megatron_neutrino_optimizer is None:
        pytest.skip("Neutrino is not in this tree")
...
@pytest.mark.skipif(WORLD_SIZE < 4, reason="needs 4 ranks")
@pytest.mark.skipif(get_megatron_neutrino_optimizer is None, reason="Neutrino is not in this tree")
